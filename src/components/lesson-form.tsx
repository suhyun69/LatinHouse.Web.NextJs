...
import { cn, toKstDateTimeString, isEmpty, isValidNonNegativeNumber, isValidTimeFormat, getGenderByGenreText, getDiscountText, validateInstructors } from "@/lib/utils"
...
export function LessonForm({ lesson, loginProfile, friends, onSubmit, onUploadImage }: LessonFormProps) {
    ...
    if (discountType === "gender") {
      return (
        <Select value={discountCondition} onValueChange={setDiscountCondition}>
          ...
          <SelectContent>
            <SelectItem value="Male">{getGenderByGenreText("M", genre)}</SelectItem>
            <SelectItem value="Female">{getGenderByGenreText("F", genre)}</SelectItem>
          </SelectContent>
        </Select>
      )
    }
